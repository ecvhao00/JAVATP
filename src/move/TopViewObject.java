...
public class TopViewObject extends ObjectByKey
{
        ...
        private final int culpritIndex = 1; // 두 번째 용의자가 범인
        ...
        public String getAccusationMessage() {
                return accusationMessage;
        }
        ...
        // 선택한 번호에 따라 범인 여부를 판별
        public void chooseSuspect( int index ) {
                if ( !awaitingAccusation || accusationResolved )
                        return;
                if ( ( index < 0 ) || ( index >= suspects.length ) )
                        return;
